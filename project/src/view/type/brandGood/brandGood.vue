<template>
	<div class="brandGood">
		<h3 class="brandGoodTitle">品牌精选</h3>
		<div
		class="brandGoodHeadImg"
		>
			<img @click="goTobrandGoodImg" :src="imdData.src" />
		</div>
		
		<ul class="brandGoodList">
			<router-link 
			tag="li"
			v-for="item,index in dataList"
			:key='index'
			:to="{name:'Bannerphone',params:{ids:item.id}}"
			>
				<img class="brandGoodItemImg" :src="item.sku_info[0].ali_image"/>
				<span class="brandGoodItemTitle">{{item.shop_info.spu_title}}</span>
				<span class="goToLoockIco"></span>
			</router-link>

		</ul>
	</div>
</template>

<script>
	export default {
		data(){
			return {
				imdData:[],
				idList:[],
				dataList:[]
			}
		},
		async created(){
			let {data} = await this.$api.getTypeData()
			this.imdData = data[7].image
			this.idList = data[7].layout.dataList.map(item => item.sku.slice(item.length-9).slice(0,7))

			let datas = await this.$api.getBannerPhone(this.idList)
			this.dataList = datas.data.data.list
		},
		methods:{
			goTobrandGoodImg(){
				this.$router.push({
					name:'Navitem',
					params:{id:61}
				})
			}
		}
	}
</script>

<style>
	.brandGood{
		background:#fff;
	}
	.brandGood .brandGoodTitle{
		height:2.4rem;
		line-height:2.4rem;
		font-size:.95rem;
		font-weight:700;
		color:#666;
		padding-left:.7rem;
		border-bottom:1px solid #f5f5f5;
	}
	.brandGood .brandGoodHeadImg{
		width:100%;
		padding:.7rem .7rem;
		box-sizing:border-box;
		border-bottom:1px solid #f5f5f5;
	}
	.brandGood .brandGoodHeadImg img{
		background-size:contain|cover;
		width:100%;
		height: auto;
	}
	.brandGood .brandGoodList{
		overflow:hidden;
	}
	.brandGood .brandGoodList li{
		position:relative;
		float:left;
		display:flex;
		align-items:center;
		width:50%;
		height:5rem;
		box-sizing:border-box;
		border-right:1px solid #f5f5f5;
		border-bottom:1px solid #f5f5f5;
	}
	.brandGood .brandGoodList li .brandGoodItemImg{
		float:left;
		width:2.5rem;
		height:2.5rem;
		margin:0 .6rem;
	}
	.brandGood .brandGoodList li .brandGoodItemTitle{
		float:left;
		overflow:hidden;
		text-overflow:ellipsis;
		line-height:1.2;
		font-weight:700;
		font-size:.8rem;
		color:#666;
		white-space:nowrap;
		padding-right:1.5rem;
		box-sizing:border-box;
	}
	.brandGood .brandGoodList li .goToLoockIco{
		position:absolute;
		top:40%;
		right:.5rem;
		width:1rem;
		height:1rem;
		background:url(data:image/png;base64,) no-repeat 50%;
		background-size:contain;
	}
</style>