<template>
	<div class="stationery">
		<h3 class="stationeryTitle">文具</h3>
		<div
		class="stationeryHeadImg"
		>
			<img @click="goTostationeryImg" :src="imdData.src" />
		</div>
		
		<ul class="stationeryList">
			<router-link 
			tag="li"
			v-for="item,index in dataList"
			:key='index'
			:to="{name:'Bannerphone',params:{ids:item.id}}"
			>
				<img class="stationeryItemImg" :src="item.sku_info[0].ali_image"/>
				<span class="stationeryItemTitle">{{item.shop_info.spu_title}}</span>
				<span class="goToLoockIco"></span>
			</router-link>

		</ul>
	</div>
</template>

<script>
	export default {
		data(){
			return {
				imdData:[],
				idList:[],
				dataList:[]
			}
		},
		async created(){
			let {data} = await this.$api.getTypeData()
			this.imdData = data[6].image
			this.idList = data[6].layout.dataList.map(item => item.sku.slice(item.length-9).slice(0,7))

			let datas = await this.$api.getBannerPhone(this.idList)
			this.dataList = datas.data.data.list
		},
		methods:{
			goTostationeryImg(){
				this.$router.push({
					name:'Navitem',
					params:{id:75}
				})
			}
		}
	}
</script>

<style>
	.stationery{
		background:#fff;
	}
	.stationery .stationeryTitle{
		height:2.4rem;
		line-height:2.4rem;
		font-size:.95rem;
		font-weight:700;
		color:#666;
		padding-left:.7rem;
		border-bottom:1px solid #f5f5f5;
	}
	.stationery .stationeryHeadImg{
		width:100%;
		padding:.7rem .7rem;
		box-sizing:border-box;
		border-bottom:1px solid #f5f5f5;
	}
	.stationery .stationeryHeadImg img{
		background-size:contain|cover;
		width:100%;
		height: auto;
	}
	.stationery .stationeryList{
		overflow:hidden;
	}
	.stationery .stationeryList li{
		position:relative;
		float:left;
		display:flex;
		align-items:center;
		width:50%;
		height:5rem;
		box-sizing:border-box;
		border-right:1px solid #f5f5f5;
		border-bottom:1px solid #f5f5f5;
	}
	.stationery .stationeryList li .stationeryItemImg{
		float:left;
		width:2.5rem;
		height:2.5rem;
		margin:0 .6rem;
	}
	.stationery .stationeryList li .stationeryItemTitle{
		float:left;
		overflow:hidden;
		text-overflow:ellipsis;
		line-height:1.2;
		font-weight:700;
		font-size:.8rem;
		color:#666;
		white-space:nowrap;
		padding-right:1.5rem;
		box-sizing:border-box;
	}
	.stationery .stationeryList li .goToLoockIco{
		position:absolute;
		top:40%;
		right:.5rem;
		width:1rem;
		height:1rem;
		background:url(data:image/png;base64,) no-repeat 50%;
		background-size:contain;
	}
</style>