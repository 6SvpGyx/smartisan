<template>
	<div class="phone">
		<h3 class="phoneTitle">手机</h3>
		<div
		class="phoneHeadImg"
		>
			<img @click="goToPhoneImg" :src="imgData.image&&imgData.image.src" />
		</div>
		<ul class="phoneList">
			<router-link
			tag="li"
			v-for="item,index in dataList"
			:key="index"
			:to="{name:'Bannerphone',params:{ids:item.id}}"
			>
				<img :src="item.sku_info[0].ali_image" />
				<span class="phoneItemTitle">{{item.shop_info.spu_title}}</span>
				<div class="buyZ">买赠</div>
				<span class="goLoock"></span>
			</router-link>
		</ul>
	</div>
</template>

<script>
	export default {
		data(){
			return {
				imgData:{},
				idList:[],
				dataList:[]
			}
		},
		methods:{
			goToPhoneImg(){
				this.$router.push({
					name:'Bannerphone',
					params:{
						ids:this.imgData.image&&this.imgData.image.linkUrl.slice(this.imgData.image.linkUrl.length-7)
					}
				})
			}
		},
		async created(){
			let {data} =await this.$api.getTypeData()
			this.imgData = data[0]
			this.idList = data[0].layout.dataList.map(item=>item.sku).map(item=>item.slice(0,7))

			let datas = await this.$api.getBannerPhone(this.idList)
			this.dataList = datas.data.data.list
		}
	}
</script>

<style>
	.phone{
		background:#fff;
	}
	.phone .phoneTitle{
		height:2.4rem;
		line-height:2.4rem;
		font-size:.95rem;
		font-weight:700;
		color:#666;
		padding-left:.7rem;
		border-bottom:1px solid #f5f5f5;
	}
	.phone .phoneHeadImg{
		width:100%;
		padding:.7rem .7rem;
		box-sizing:border-box;
		border-bottom:1px solid #f5f5f5;
	}
	.phone .phoneHeadImg img{
		background-size:contain|cover;
		width:100%;
		height: auto;
	}
	.phone .phoneList{
		font-size:.8rem;
		line-height:3.2;
		font-weight:700;
		color:rgba(0,0,0,.8);

	}
	.phoneList li{
		position:relative;
		height:4rem;
		padding:.6rem 0 0 0;
		box-sizing:border-box;
		border-bottom:1px solid #f5f5f5;
		overflow:hidden;
	}
	.phoneList li img{
		float:left;
		width:2.5rem;
		height:2.5rem;
		margin:0 .6rem;
		vertical-align:top;
	}
	.phoneList li .phoneItemTitle{
		float:left;
	}
	.phoneList li .buyZ{
		float:left;
		width:2.5rem;
		height:.8rem;
		border:1px solid #cc3e39;
		color:#cc3e39;
		line-height:.8rem;
		text-align:center;
		font-size:.3rem;
		margin:.8rem 0 0 .8rem;
		border-radius:.7rem;
		background:#ffe7e5;
	}
	.phoneList li .goLoock{
		display:block;
		width:1rem;
		height:1rem;
		
		position:absolute;
		top:30%;
		right:.5rem;
		background:url(data:image/png;base64,) no-repeat 50%;
		background-size:contain;
	}
</style>